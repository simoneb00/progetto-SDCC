...
if [ "$(docker ps -aq -f name="main_container")" ]; then
  ...
  docker run -d --privileged=true  -v /var/run/dbus/system_bus_socket:/var/run/dbus/system_bus_socket -v /var/run/docker.sock:/var/run/docker.sock -e CONTAINER_NAME="main_container" -e PYTHONUNBUFFERED=1 --name "main_container" "main_image"
